...
public class Sorter implements Sort
{
	...
	**/
	public void quickSort(long[] a){
		...
		quickSort_instead(a, 0, a.length - 1);
	}

	public void quickSort_instead(long[] a, int p, int r) {
		if(p<r) {
			long q = partition(a,p,r);
			quickSort_instead(a,p,(int) (q-1));
			quickSort_instead(a,(int) (q+1),r);
		}
	}
	
	private static long partition(long[] a, int p, int r) {
		long x = a[r];
		int i = p-1;
		for(int j=p;j<r;j++) {
			if(a[j]<=x) {
